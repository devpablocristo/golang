...
func analyzeGoFile(filePath string, interfaces *[]Interface) error {
	...
	for scanner.Scan() {
		...
		// Buscar la declaración del paquete
		...
		if interfaceMatch := interfaceRegex.FindStringSubmatch(line); interfaceMatch != nil {
			...
			*interfaces = append(*interfaces, interf)
			fmt.Printf("Interface encontrada: %s en paquete %s, archivo %s, línea %d\n", interf.Name, currentPackage, filePath, lineNumber)
		}
	...
	for scanner.Scan() {
		...
		for _, interf := range interfaces {
			if strings.Contains(line, interf.Name) {
				foundUsages = append(foundUsages, fmt.Sprintf("Uso de interfaz %s (paquete: %s) en línea %d", interf.Name, interf.Package, lineNumber))
			}
		}
