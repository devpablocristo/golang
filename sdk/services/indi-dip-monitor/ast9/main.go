	...
	func listVariablesStructsParamsAndInterfaces(filePath string) error {
		...
		localVariables := make(map[string]bool) // Track local variables
		...
		ast.Inspect(file, func(n ast.Node) bool {
			// Only consider top-level variable declarations as global variables
			if decl, ok := n.(*ast.GenDecl); ok && decl.Tok == token.VAR {
				if _, inFunction := findFunctionScope(n); !inFunction {
					// We are outside any function or method, so these are global variables
					for _, spec := range decl.Specs {
						if vspec, ok := spec.(*ast.ValueSpec); ok {
							for _, name := range vspec.Names {
								// If this variable was declared locally, skip it
			...
			// Look for function declarations and handle local variables within the function body
							...
							for _, spec := range genDecl.Specs {
								if vspec, ok := spec.(*ast.ValueSpec); ok {
									for _, name := range vspec.Names {
										...
										// Register the variable as local
	...
	// Helper function to determine if we are inside a function scope
