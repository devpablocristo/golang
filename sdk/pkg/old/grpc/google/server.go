...
func InitializeGgrpcServer(config portspkg.GgrpcConfig) error {
	onceServer.Do(func() {
		...
		if config.GetTLSConfig() != nil {
			...
			if err != nil {
				initErrorServer = fmt.Errorf("failed to load TLS config: %v", err)
				return
			}
		...
		if err != nil {
			initErrorServer = fmt.Errorf("failed to listen: %v", err)
			return
		}
