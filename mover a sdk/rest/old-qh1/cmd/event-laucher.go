...
import (
	...
	handler "github.com/devpablocristo/qh/cmd/handlers"
...
type EventLauncher struct {
	...
	router      handler.EventRouterPort
	dao         event.MongoEventDAOPort
	repo        event.RepoPort
	useCase     event.UseCasePort
	handlerr    handler.EventHandlerPort
	running     bool
}
...
func (l *EventLauncher) setupEventComponents() {
	...
	l.handlerr = handler.NewEventHandler(l.useCase)
	l.router = handler.NewEventRouter(l.handlerr)
}
