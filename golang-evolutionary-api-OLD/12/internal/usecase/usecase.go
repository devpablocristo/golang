...
type ItemUsecasePort interface {
	...
	GetItem(domain.ID) (*domain.Item, error)
}
...
// GetItem retrieves an item by its ID, returning a custom error if not found.
func (u *ItemUsecase) GetItem(id domain.ID) (*domain.Item, error) {
	item, err := u.repository.GetItem(id)
	if err != nil {
		return nil, ctypes.NewCustomError(1, ctypes.ErrItemNotFound)
	}
	return item, nil
